...
const getImagesData = () => {
  ...
  return unirest
    .get(
      "https://www.google.com/search?q=keith-hetrick&oq=keith-hetrick&hl=en&tbm=isch&asearch=ichunk&async=_id:rg_s,_pms:s,_fmt:pc&sourceid=chrome&ie=UTF-8"
    )
    .headers(header)
    .then((response) => {
      ...
      // create text file & append response data on a new line
      const fs = require("fs");
      fs.appendFile(
        "googleImagesResults.txt",
        JSON.stringify(images_results),
        function (err) {
          if (err) throw err;
          console.log("Saved!");
        }
